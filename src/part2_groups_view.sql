DROP FUNCTION IF EXISTS find_margin() CASCADE;

CREATE OR REPLACE FUNCTION find_margin(mode int DEFAULT 1, option int DEFAULT 5000)
RETURNS TABLE (customer_id_new bigint, group_id_new bigint, Group_Margin double precision) AS $$
    BEGIN
        IF (mode = 1) THEN
...
WITH find_group_id AS (SELECT DISTINCT customer_id, group_id
                       FROM (SELECT cards.customer_id, sku_id
                             ...
                             WHERE transaction_datetime <
                                   (SELECT analysis_formation FROM date_of_analysis_formation)) AS sku_id
                       JOIN product_grid ON sku_id.sku_id = product_grid.sku_id),

     find_Group_Affinity_Index AS (SELECT cnt_trans.customer_id, group_id,
                                          (group_purchase::double precision / cnt::double precision) AS Group_Affinity_Index
     ...
     find_Group_Discount_Share AS (SELECT DISTINCT periods_view.customer_id, periods_view.group_id,
        ...
        OVER (PARTITION BY periods_view.customer_id, periods_view.group_id)::double precision / group_purchase AS Group_Discount_Share
                                   FROM checks
